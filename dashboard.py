        ...
        for l100 in _controlsState:
          if lateral_type == "":
            if l100.controlsState.lateralControlState.which == "pidState":
              lateral_type = "pid"
              influxFormatString = user_id + ",sources=capnp angle_steers_des=%s,dampened_angle_steers_des=%s,angle_steers=%s,steer_override=%s,v_ego=%s,p=%s,i=%s,f=%s,ff_angle=1.0,output=%s %s\n"
              kegmanFormatString = user_id + ",sources=kegman KpV=%s,KiV=%s,Kf=%s %s\n"
            else:
              lateral_type = "indi"
              influxFormatString = user_id + ",sources=capnp angle_steers_des=%s,angle_steers=%s,damp_angle_steers_des=%s,steer_override=%s,v_ego=%s,output=%s,indi_angle=%s,indi_rate=%s,indi_rate_des=%s,indi_accel=%s,indi_accel_des=%s,accel_error=%s,delayed_output=%s,indi_delta=%s %s\n"
          ...
          if vEgo > 0 and active:
            ...
            if lateral_type == "pid":
              influxDataString += ("%0.3f,%0.2f,%0.2f,%d,%0.1f,%0.4f,%0.4f,%0.4f,%0.4f,%d|" %
